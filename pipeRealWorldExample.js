...
const sendPushNotification = ({title,body, users, user}) => Promise.resolve(console.log(`${title} ${body} ${users} ${user}`));
//envio de un mensaje a un canal pusher
const pusherTrigger = (person)  => Promise.resolve(console.log(`mensaje pusher enviado ${JSON.stringify(person)}`))
const pusherTriggerError = () => {throw new Error('se murio pusher')}
const generateQueueMessage = (options = {}) => options
//envio de un sms
const sendLodgingConfirmationSms = (message)  => Promise.resolve(console.log(`mensaje de confirmacion enviado ${JSON.stringify(message)}`))
...
const prepareBeamData = (person,filteredLodgings) => {
    ...
    return { title, body, users: uniqOwnerIds, user: 'microservicio de notificaciones' };
};
...
const pipeExample = async (person, filteredLodgings, from) => {
  try {
    ...
    console.error(`error encontrado en una de las funciones de pipe ${e.message}`)
  }
}
